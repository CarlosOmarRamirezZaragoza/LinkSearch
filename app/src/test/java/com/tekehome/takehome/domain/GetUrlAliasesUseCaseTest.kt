...
class GetUrlAliasesUseCaseTest {
    @get:Rule
    ...
    @Inject
    lateinit var getUrlAliasesUseCase: GetUrlAliasesUseCase
    ...
    fun `Post creating aliases Use Case`() = runTest {
        val getUrlAliases = getUrlAliasesUseCase(ALIAS)
        advanceUntilIdle()
        getUrlAliases.collect { url ->
            Assert.assertEquals(url, NU_URL)
        }
    }
}
